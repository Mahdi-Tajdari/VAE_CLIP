...
def load_random_image(image_paths, size=256, device="cuda"):  # Changed to 256
...
def train_auxiliary_network(
    ...
    num_epochs=10, steps_per_epoch=10,
    lr=1e-4, log_every=1, ckpt_every=5,
    device=None
):
    ...
    with torch.no_grad():
        txt_inputs = clip_processor(
            text=["a close-up of a person with clear glasses"], return_tensors="pt", padding=True
        ).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step in range(steps_per_epoch):
            imgs = load_random_image(image_paths, size=256, device=device)
            ...
            total_loss, feature_loss, identity_loss = criterion(
                im_init_clip, im_edit_clip, text_embeds_target, alpha=0.95
            )
            ...
            if step == 0:
                dz_fixed = torch.ones_like(z) * 0.5
                z_prime_fixed = z + dz_fixed
                img_edit_fixed = vae.decode(z_prime_fixed / SD_SCALE).sample.float()
                save_image((img_edit_fixed + 1) / 2, f"inference_samples/test_fixed_epoch_{epoch}.png")
                print(f"Test with fixed dz saved as test_fixed_epoch_{epoch}.png")
        ...
        if epoch % ckpt_every == 0:
            ...
            with torch.no_grad():
                test_img = load_random_image(image_paths, size=256, device=device)
